...
class TraktApi:
    ...
    def revoke_session(self) -> bool:
        ...
        self.post('oauth/revoke', credentials=cred, login_required=True, data={
    ...
    def get_watched_list(self, type: WatchContentType, *,
        ...
        resp = self.get(f'sync/watched/{type}', errors=errors, extended=extended, login_required=True)
        return resp if isinstance(resp, Sequence) else ()
    ...
    def media(self, type: MainMediaType, id: Union[str, int], *, errors: str = 'ignore') -> JsonData | None:
    ...
    def user_profile(self, user: str = 'me', *, credentials: TraktCredentials | None = None) -> Optional[TraktUserProfile]:
    ...
    def user_generic_list(self,
        ...
        if page == ALL_PAGES:
            with self.depagination() as trakt:
                data, pages = trakt.get_with_pages('/'.join(path), params=params, extended=extended, login_required=login_required)
        else:
            data, pages = self.get_with_pages('/'.join(path), params=params, extended=extended, login_required=login_required)
        if isinstance(data, Sequence):
    ...
    def user_collection(self,
        ...
        def get(mtype: MainMediaType) -> Sequence[JsonData]:
            ...
            data = self.get(f'users/{user}/collection/{ctype}', login_required=(user == 'me'), extended=extended)
            if isinstance(data, Sequence):
                return data
            return []
