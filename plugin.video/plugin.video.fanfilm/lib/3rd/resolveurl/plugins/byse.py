...
class ByseResolver(ResolveUrl):
    ...
    def er(self, t, e, r=180.0):
        ...
        try:
            ...
            so_path = os.path.join(curr_dir, 'libbyse.so')
            if os.path.exists(so_path):
                lib = ctypes.CDLL(so_path)
                lib.solve_challenge.argtypes = [ctypes.c_char_p, ctypes.c_int, ctypes.c_char_p]
                lib.solve_challenge.restype = ctypes.c_int
                
                result_buf = ctypes.create_string_buffer(64)
                prefix_bytes = (t + ':').encode('latin-1')
                
                success = lib.solve_challenge(prefix_bytes, e, result_buf)
                if success:
                    return result_buf.value.decode('latin-1')
        except Exception:
            pass
