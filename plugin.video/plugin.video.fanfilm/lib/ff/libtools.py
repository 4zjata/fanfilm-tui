...
class LibTools:
    ...
    def check_in_library(
        ...
        *,
        ...
        if all_episodes:
            fflog(f'Sprawdzanie serialu z ID: {tmdb}/{imdb} z rokiem: {year}')
            tvshow = find_tvshow()
            if tvshow:
            ...
            fflog(f'Serial z ID: {tmdb}/{imdb} nie znaleziony w bibliotece.')
            return None

        elif season is not None and episode is not None:
            fflog(f'Sprawdzanie odcinka S{season}E{episode} serialu z ID: {tmdb}/{imdb}')
            tvshow = find_tvshow()
            if tvshow:
                ...
                fflog(f'Serial z ID: {tmdb}/{imdb} nie znaleziony.')
            return None

        else:
            fflog(f'Sprawdzanie filmu z ID: {tmdb}/{imdb} z rokiem: {year}')
            try:
                ...
                fflog(f'Film z ID: {tmdb}/{imdb} nie znaleziony.')
            except Exception:
                fflog(f'Błąd podczas sprawdzania filmu z ID: {tmdb}/{imdb}')
            return None
    ...
    def add(self, items: FFRef | Iterable[FFRef], *, reload: bool = True) -> None:
