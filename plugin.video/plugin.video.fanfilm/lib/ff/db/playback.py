...
from .orm import OrmDatabase, DbTable, field as db_field, relationship, select, update, AND, OR, IN, MISSING
...
from ..calendar import utc_timestamp, utc_from_timestamp, datestamp as make_datestamp, is_datetime_instance
...
class MediaPlayService(DbTable):
    """Scrobble (e.g. trakt) service description row."""

    __tablename__ = 'scrobble_service'
    # UNIQUE(type, ffid)
    __table_args__ = ('UNIQUE("name")',)

    #: Service database ID.
    id: PrimaryKey = None
    #: Service name (e.g. 'trakt').
    name: str


class MediaPlayInfo(DbTable):
    """Scrobble (e.g. trakt) playback row."""

    __tablename__ = 'scrobble_playback'
    # UNIQUE(type, ffid)
    __table_args__ = ('UNIQUE("xref::type", "xref::ffid", "xref::season", "xref::episode")',)

    #: Playback database ID.
    id: PrimaryKey = None
    #: Media reference (with -1 in season and episode instead of None).
    xref: XMediaRef = MISSING
    #: Denormalized media FFID (e.g. episode FFID for show episodes).
    ffid: int = 0
    ...
    #: TV show Trakt.tv slug.
    tv_slug: Optional[str] = None
    #: Scrobble service ID
    service_id: Optional[int] = db_field(foreign_key='scrobble_service.id')
    #: Scrobble service object (based on `service_id`).
    service: Optional[MediaPlayService] = relationship()
    #: Level of details stored in the playback info.
    details_level: int = 0
    #: Playback unique ID (from scrobble service).
    playback_id: Optional[int] = None
    #: Episode progress in percent. If media is watched, progress is None.
    progress: Optional[float] = None
    #: Episode progress bar, ex. 100101110.
    progress_map: Optional[str] = None
    #: Timestamp (UTC unixtime seconds) where playback was paused.
    paused_at: Optional[int] = None
    #: Number of times the media was played (watched).
    play_count: Optional[int] = None
    #: Timestamp (UTC unixtime seconds) when the media was last watched.
    last_watched_at: Optional[int] = None
    #: Media duration in seconds.
    duration: Optional[int] = None
    #: Date stamp (number of days from 1 Jan AD 1) when the media was aired (released).
    aired_at: Optional[int] = None
    #: Date stamp (number of days from 1 Jan AD 1) when the next episode will be aired.
    next_aired_at: Optional[int] = None  # use only in tv-show or season
    #: Metadata last updated timestamp (UTC unixtime seconds).
    ...
    def __init__(self, *, id: PrimaryKey = None, ref: MediaRef = MISSING, xref: Union[XMediaRef, MissingType] = MISSING, ffid: int = 0,
                 ...
                 slug: Optional[str] = None, tv_slug: Optional[str] = None, service_id: Optional[int] = None, service: Optional[MediaPlayService] = None,
                 details_level: Optional[int] = None, playback_id: Optional[int] = None,
        ...
        if service_id is not None and service is not None:
            raise ValueError('cannot specify both `service_id` and `service`')
        if service is not None:
            service_id = service.id
        if service_id is None and main_srobble_service is not None:
            service_id = main_srobble_service.id  # default service ID (trakt)

        if details_level is None:
            details_level = 1  # default details level (degraded)
        ...
        self.service_id       = service_id                       # noqa: E221
        self.details_level    = int(details_level or 0)          # noqa: E221
...
def on_update(cur: DbCursor) -> None:
    """Update DB schema to the current version, force recount progress for shows and seasons."""
    playbacks = _get_playback(cur, type='show')
    shows = [ref for ref in playbacks.keys() if ref.is_show or ref.is_season]
    _fix_shows_progress(cur, reset=shows, playbacks=playbacks)


def on_connect(cur: DbCursor) -> None:
    """Get main info from DB, called on DB connect."""
    global main_srobble_service
    main_srobble_service = cur.exec(select(MediaPlayService).where(MediaPlayService.name == 'trakt')).first()


#: Main scrobble service (e.g. trakt).
main_srobble_service: Optional[MediaPlayService] = None

#: Global playback DB.
db = OrmDatabase('playback', MediaPlayInfo, MediaPlayService, version=DB_VERSION, on_update=on_update, on_connect=on_connect)
...
def _get_shows_info(refs: Iterable[MediaRef]) -> FFItemDict:
    ...
    return ffinfo.get_item_dict(tuple(shows), progress=ffinfo.Progress.NO, details=const.media.progress.info_details)
...
def remove_all_track_watched_items() -> None:
    """Remove all items, clear playback table."""
    with db.cursor() as cur:
        cur.execute(f'DELETE FROM {MediaPlayInfo.__tablename__}')
