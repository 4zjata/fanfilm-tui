...
from ..types import is_optional, remove_optional, is_union_none, get_annotations, ReferenceType, TypeForm, AnnMeta, ann_type, find_arg_descr, PY38_TYPES
...
def field(*,  # type: ignore  ("xfield" is marked as overload, but no implementation is provided )
    ...
    meta[AttrsFieldExtension.METADATA] = AttrsFieldExtension(dump=dump, load=load, on_update=on_update,
                                                             auto_generated=auto_generated, foreign_key=foreign_key)
...
class Relationship(Generic[TI, V]):
    ...
    def __repr__(self) -> str:
        return f'Relationship(name={self.name!r}, back_populates={self.back_populates!r}, on={self._on!r}, when={self.when!r}, is_table={self.is_table!r}, ann={self.ann!r})'
                ...
                if cur:
                    ...
                    if isinstance(origin, type) and issubclass(origin, Sequence):
                        ...
                        if isinstance(typ, type) and issubclass(typ, DbTable) and self.back_populates:
                            ...
                            return val
        # relationship in where()
        if obj is None and objtype._fields_info and (field := objtype._fields_info.get(self.on)):
            foreign_key = field.foreign_key
            if not foreign_key and self.on and self.on.endswith('_id'):
                foreign_key = f'{self.on[:-3]}.{objtype.__primary_index__ or "id"}'
            return TableColumnExpr(table=objtype, field=field, on=foreign_key)
        return None
...
class DbTableMeta(type):
    ...
    def __new__(cls, name, bases: Tuple[Type[Any], ...], dct: Dict[str, Any], *, table: bool = True, _base_orm_class: bool = False):
        ...
        if False:
            # could not be resovled here because of possible forward references
            if table and not _base_orm_class:
                kls._fields  # force fields initialization
        return kls
    ...
    #     # print(f' > meta.__init_subclass__({cls=}, {kwargs=})')
    ...
    def _fields(self) -> Dict[str, DbField]:
        ...
        def create_field(field: attr.Attribute, ann: Dict[str, TypeForm[Any]], *, optional: bool = False, prefix: str = '',
            ...
            if attrs.has(typ) or dc.is_dataclass(typ) or is_namedtuple_class(typ):
                if typ in used:
        ...
        if tab._fields_info is None:
            try:
                ...
                ann = get_type_hints(self, {**tp.__dict__, **PY38_TYPES, **globals()}, sys.modules[tab.__module__].__dict__, include_extras=True)
            if attrs.has(self):
                ...
                fields = ()
            tab._fields_info = {f.name: create_field(f, ann) for f in fields if not f.name.startswith('_') and not isinstance(f.default, Relationship)}
            # ???
...
class DbTable(metaclass=DbTableMeta, table=False, _base_orm_class=True):
    ...
    @classmethod
    def model_init(cls) -> None:
        """Initialize model (force fields initialization)."""
        cls._fields  # force fields initialization
...
class Expr:
    ...
    def __init__(self, scalar: Any = MISSING, *, op: str = '', value: str = '', second: str = '', on: Optional[str] = None) -> None:
        self.op: str = op
        self.value: str = value if scalar is MISSING else sql_dump(scalar)
        self.second: str = second
        #: what to extract from the second value (for relationships)
        self.on: Optional[str] = on
    ...
    def _rel(self, other: Any) -> Any:
        """Get relationship value."""
        if isinstance(other, DbTable):
            if self.on and other._fields_info:
                table, _, key = self.on.partition('.')
                if other.__tablename__ == table and (field := other._fields_info.get(key)):
    ...
    def __matmul__(self, other: Union[Iterable[Any], 'Statement']) -> 'Expr':
        if self.on and not isinstance(other, Statement):
            other = [self._rel(o) for o in other]
...
class TableColumnExpr(Expr, Generic[TABLE]):
    ...
    def __init__(self, *, table: TABLE, field: DbField, on: Optional[str] = None) -> None:
        self.table: Type[DbTable] = table
        self.field: DbField = field
        if field.fields is None:
            super().__init__(value=f'{quote(self.table.__tablename__)}.{quote(self.field.column)}', on=on)
        else:
...
class OrmDatabase:
    ...
    def __init__(self,
                 ...
                 *classes: Type[DbTable],
                 ...
                 on_update: Optional[Callable[[DbCursor], None]] = None,
                 ) -> None:
        self.name = name
    ...
    def _on_connect(self, cur: DbCursor) -> None:
        ...
        if old_ver is None and callable(self.on_create):
            self.on_create(cur)
        elif old_ver != self.version and callable(self.on_update):
            self.on_update(cur)
