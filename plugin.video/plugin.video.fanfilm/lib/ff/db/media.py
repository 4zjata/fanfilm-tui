...
from .db import db_manager, update_db_version, db_create_columns, db_list_columns, Lock, sql_dump, DbCursor
...
@contextmanager
def get_cursor(name: str = 'media') -> Iterator[DbCursor]:
...
@contextmanager
def media_cursor() -> Iterator[DbCursor]:
    """Get "state" cursor in `with` statement."""
    with get_cursor('media') as cur:
        yield cur


def _update_db(cur: DbCursor, ver: int) -> None:
