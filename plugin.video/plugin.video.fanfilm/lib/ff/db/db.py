...
try:
    ...
    DB_PAlib: Path = FF3_PATH
    # DB_PATH: Path = Path('~/.kodi/userdata/addon_data/plugin.video.fanfilm').expanduser()  # DEBUG (terminal)

from ..types import is_optional, remove_optional, is_datetime_instance
from ..tricks import singleton, is_namedtuple, MISSING, JsonEncoder
...
Unique: TypeAlias = Annotated[T, 'UNIQUE']
ColumnName = str
...
def sql_dump(v: Any, *, _inner: bool = False) -> str:
    ...
    if isinstance(v, (Expr, Statement)):
        return str(v)
    if attrs.has(type(v)):
        vv = ', '.join(sql_dump(getattr(v, f.name), _inner=True) for f in attrs.fields(type(v)))
        if _inner:
            return vv
        return f'({vv})'
    if is_dataclass(v):
    ...
    if isinstance(v, SQL):
        return v
    if isinstance(v, (dt_date, dt_time)) or is_datetime_instance(v):
        v = str(v)
    return repr(v)
...
def dump_value_and_type(value: Any, *, mode: Optional[Union[TypeMode, str]] = None, force_str: bool = False) -> Tuple[Any, str]:
    ...
    if type(value) in known_types:
        ...
        if serializer := getattr(value, '__to_json__', None):
            ...
            value = json.dumps(attrs.asdict(value), cls=JsonEncoder)
    elif serializer := getattr(value, '__to_json__', None):
        type_name, value = TypeMode.Json, serializer()
    elif attrs.has(value):
        type_name, value = TypeMode.Json, json.dumps(attrs.asdict(value), cls=JsonEncoder)
