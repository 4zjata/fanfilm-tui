...
class Addon:
    ...
    def getSettings(self) -> 'Settings':
        if self._settings is None:
            ...
            self._settings = Settings(path, addon_path=self.path)
    ...
    def setSetting(self, id: str, value: str) -> None:
        deprecated_warning()
        self.setSettingString(id, value)
...
class Settings:
    ...
    def __init__(self, path: Optional[Path] = None, addon_path: Optional[Path] = None) -> None:
        ...
        if addon_path:
            settings_xml_path = addon_path / 'resources' / 'settings.xml'
            if settings_xml_path.exists():
                try:
                    self._setting_defs = _etree.parse(str(settings_xml_path)).getroot()
                except Exception:
                    pass
        ...
        self._types = {}
        self._defaults = {}
        if self._setting_defs is not None:
            for s in _xpath(self._setting_defs, './/setting'):
                sid = s.attrib.get('id')
                if sid:
                    self._types[sid] = s.attrib.get('type', 'string')
                    # Kodi v2 settings use a <default> child element; fall back to attribute
                    default_el = s.find('default')
                    if default_el is not None and default_el.text is not None:
                        self._defaults[sid] = default_el.text
                    else:
                        self._defaults[sid] = s.attrib.get('default', '')
    ...
    def getInt(self, id: str) -> int:
        val = self.getString(id)
        try:
            return int(val)
        except ValueError:
            default_val = self._defaults.get(id, '')
            try:
    ...
    def setString(self, id: str, value: str) -> None:
        ...
        try:
            ...
            node = _etree.SubElement(self._root, 'setting', id=id)
        if node.text == value:
            return
        self._dirty = True
        node.text = str(value)
