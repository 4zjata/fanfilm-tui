...
def get_active_captcha_image():
    for dialog in reversed(_active_dialogs):
        if getattr(dialog, 'visible', False):
            for control in getattr(dialog, 'controls', []):
                if control.__class__.__name__ == 'ControlImage' and getattr(control, 'filename', None):
...
class Dialog:
    ...
    def yesno(self, heading: str,
        ...
        if _tui_app is not None:
            ...
            def callback(ans):
                result[0] = ans
                event.set()
            _tui_app.call_from_thread(_tui_app.push_yesno, heading, message, callback)
            event.wait()
            return result[0]
        else:
    ...
    def ok(self, heading: str, message: str) -> bool:
        if _tui_app is not None:
            ...
            def callback():
                event.set()
            _tui_app.call_from_thread(_tui_app.push_ok, heading, message, callback)
            event.wait()
            return True
        else:
            print(f'== {heading} ==\n{message}')
    ...
    def notification(self, heading: str,
        ...
        if _tui_app is not None:
            _tui_app.call_from_thread(_tui_app.tui_notification, heading, message, icon, time, sound)
        else:
    ...
    def input(self, heading: str,
              ...
              autoclose: int = 0) -> str:
        if _tui_app is not None:
            captcha_image = get_active_captcha_image()
            event = threading.Event()
            result = [defaultt]
            def callback(text):
                result[0] = text
            ...
            def cancel_callback():
                ...
                event.set()
            if captcha_image:
                _tui_app.call_from_thread(_tui_app.push_text_captcha, captcha_image, heading, callback, cancel_callback)
            else:
                _tui_app.call_from_thread(_tui_app.push_text_input, heading, defaultt, callback, cancel_callback)
            event.wait()
            return result[0]
        else:
...
class Window:
    ...
    def show(self) -> None:
        self.visible = True
        if self not in _active_dialogs:
    ...
    def removeControl(self, pControl: Control) -> None:
        if pControl in self.controls:
            self.controls.remove(pControl)

    def removeControls(self, pControls: List[Control]) -> None:
        for p in pControls:
            if p in self.controls:
                self.controls.remove(p)
    ...
    def doModal(self) -> None:
        self.show()
        if _tui_app is not None and hasattr(self, 'temp_file'):
            event = threading.Event()
            def callback(x, y):
                self.solution_x = x
                self.solution_y = y
            ...
            def cancel_callback():
                self.finished = False
                event.set()
            _tui_app.call_from_thread(_tui_app.push_coordinate_captcha, self.temp_file, callback, cancel_callback)
            event.wait()
        self.close()

    def addControl(self, pControl: Control) -> None:
        self.controls.append(pControl)

    def addControls(self, pControls: List[Control]) -> None:
        self.controls.extend(pControls)
