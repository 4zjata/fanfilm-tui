    ...
    def library(self):
        ...
        with directory(view=const.indexer.tools.view, thumb='common/library.png') as kdir:
            ...
            if self.lists_info.trakt_enabled():
                kdir.action(L(30314, 'Update Trakt collection'), url_for(self.sync_lib, source='trakt'), thumb='services/trakt/update.png')
            if self.lists_info.tmdb_enabled():
                kdir.action(L(30315, 'Update TMDB collection'), url_for(self.sync_lib, source='tmdb'), thumb='services/tmdb/update.png')
            if self.lists_info.imdb_enabled():
                kdir.action(L(30350, 'Update IMdb collection'), url_for(self.sync_lib, source='imdb'), thumb='services/imdb/update.png')
            if self.lists_info.mdblist_enabled():
                kdir.action(L(30538, 'Update MDBList collection'), url_for(self.sync_lib, source='mdblist'), thumb='services/mdblist/update.png')
            kdir.action(L(30539, 'Update Own Favorites'), url_for(self.sync_lib, source='own'), thumb='services/own/update.png')
