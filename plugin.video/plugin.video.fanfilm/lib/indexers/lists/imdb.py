...
class ImdbLists(Indexer):
    ...
    @route('/mine/{__page}')
    def mine(self, *, media: Optional[MainMediaType] = None, user: Optional[str] = None, page: int = 1) -> None:
        """Show IMDB lists."""
        imdb = ImdbScraper()
        users = settings.getString('imdb.user')
        if user:
            users = user
        if users and ',' in users:
            with list_directory(view=const.indexer.imdb.mine.view) as kdir:
                for it in imdb.user_info_list(users):
                    if usr := it.vtag.getIMDBNumber():
                        kdir.add(it, url=info_for(self.mine, user=usr, media=media), thumb='services/imdb/user.png')
        else:
            kwargs = {'media': media} if media else {}
            items = imdb.user_lists(users, page=page)
